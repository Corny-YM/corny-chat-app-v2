...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const passwordEncoded = handleEncodeElgamal(e.target[2].value);
    const file = e.target[3].files[0];

    // console.log(password);
    // console.log(handleDecodeElgamal(password));
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
            ...
            await createNewUser(displayName, email, passwordEncoded, downloadURL, res.user.uid);
